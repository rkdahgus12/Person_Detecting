...
labelsPath = "C:\\Users\\82108\\OneDrive\\바탕 화면\\YOLO-Object-Counting-master\\YOLO-Object-Counting-master\\yolo-coco\\coco.names"

LABELS = open(labelsPath).read().strip().split("\n")

# YOLO 가중치 및 모델 구성에 대한 경로
weightsPath = "yolov3.weights"  # 가중치
configPath = "yolov3.cfg"  # 모델 구성
...
if not args.get("input", False):
    ...
    vs.set(cv2.CAP_PROP_FPS, int(60))
...
while True:
    ...
    for output in layerOutputs:
        ...
        for detection in output:
            ...
            # 사람이 아닌 경우 제외
            # yolo-coco 디렉터리에 coco.names 파일을 참고하여 다른 object 도 인식 가능(0 인 경우 사람)
            if classID != 0:
                continue
            ...
            if confidence > args["confidence"]:
                ...
                (centerX, centerY, width, height) = box.astype(
                    "int")  # (중심 좌표 X, 중심 좌표 Y, 너비(가로), 높이(세로))
    ...
    print("People Counting : {}".format(count))
    cv2.putText(frame, counting_text, (10,
                frame.shape[0] - 25), cv2.FONT_HERSHEY_DUPLEX, 0.85, (0, 0, 255), 3)
    frame = cv2.resize(frame, dsize=(1300, 700), interpolation=cv2.INTER_AREA)
    # print(image)
    # 프레임 출력
